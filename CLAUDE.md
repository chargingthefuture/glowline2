...
## Glowline 2 — project rules

Glowline 2 is a browser game: you steer a glowing dart left-to-right through a neon maze, brush the
walls to charge a boost, and race to the finish. It is a spin on Data Wing's wall-riding navigation,
carried into the Glowline look. It runs as a static site — no build step, no server, no framework.

### Stack and layout

- Plain HTML, CSS, and JavaScript (ES modules). No bundler, no framework, no package manager.
- The whole game is static files served from the repo root, so it can be opened directly or hosted on
  GitHub Pages.
- Layout:
  - `index.html` — the page shell and the single `<canvas>`.
  - `styles/` — CSS.
  - `src/` — ES modules, one concern per file (input, physics, level data, rendering, game loop).
  - `assets/` — any images or sounds (none required to run).
- Keep each module small and single-purpose. Draw code stays in the render module; game rules stay in
  the game/ship modules; level shapes stay in the level data. Do not mix them.

### Running it

- There is no build. To play, open `index.html` in a browser, or serve the folder with any static
  server (for example `python3 -m http.server`) and open the page. ES modules need to be served over
  http(s) rather than the `file://` protocol in some browsers, so prefer a static server when testing.
- There is no automated test suite yet. When you change the physics or a level, actually play it in a
  browser and confirm the ship still completes the level before you open a pull request.

### Game feel — keep these true

- The ship always moves forward along its heading; the player steers left/right, and boosts.
- Brushing a wall at a shallow angle charges the boost meter and lets the ship slide; hitting a wall
  head-on slows it. Do not remove the wall-riding reward — it is the core of the game.
- Controls must work with both keyboard (arrows / WASD, space to boost) and touch, so the game is
  playable on a phone as well as a desktop.
- Best times are saved locally in the browser. Do not add a backend or send player data anywhere.
